var systemComponents = {
    ...
    'info-tab': {
        ...
        <br>
        The Modding Tree <a v-bind:href="'https://github.com/Acamaeda/The-Modding-Tree/blob/master/changelog.md'" target="_blank" class="link" v-bind:style = "{'font-size': '14px', 'display': 'inline'}" >{{TMT_VERSION.tmtNum}}</a>由Acamaeda制作
        <br>
        The Prestige Tree由Jacorb和Aarex制作
		<br><br>
		<div class="link" onclick="showTab('changelog-tab')">更新公告</div><br>
        <span v-if="modInfo.discordLink"><a class="link" v-bind:href="modInfo.discordLink" target="_blank">{{modInfo.discordName}}</a><br></span>
        <a class="link" href="https://discord.gg/F3xveHV" target="_blank" v-bind:style="modInfo.discordLink ? {'font-size': '16px'} : {}">The Modding Tree的Discord</a><br>
        <a class="link" href="http://discord.gg/wwQfgPa" target="_blank" v-bind:style="{'font-size': '16px'}">主Prestige Tree服务器</a><br>
		<br><br>
        已玩了:{{ formatTime(player.timePlayed) }}<br><br>
        <h3>热键</h3><br>
    ...
    'options-tab': {
        ...
        <table>
            ...
            <tr>
                <td><button class="opt" onclick="switchTheme()">主题:{{ getThemeCnName() }}</button></td>
                <td><button class="opt" onclick="adjustMSDisp()">展示里程碑:{{ MS_DISPLAYS[MS_SETTINGS.indexOf(options.msDisplay)]}}</button></td>
